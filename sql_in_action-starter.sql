...
SELECT price FROM final_airbnb ORDER BY price DESC LIMIT 1;
SELECT price FROM final_airbnb ORDER BY price ASC LIMIT 1;
...
-- <<<<<<<<<<<<<<<<<<<<<< PROBLEM 9 >>>>>>>>>>>>>>>>>>>>>>>
-- Query the data to discover which listing is the most popular using 
-- the reviews_per_month for all listings with a minimum_nights value of less than 7
...
SELECT host_id, COUNT(host_name) as num_listings
FROM final_airbnb
GROUP BY host_id
